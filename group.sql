...
AS
	SELECT MANV, HOTEN, EMAIL, CAST(CONVERT(VARCHAR(MAX), DECRYPTBYASYMKEY(ASYMKEY_ID(@TenDN), LUONG, @Matkhau)) AS INT) as LUONGCB FROM NHANVIEN
go
-- d
...
INSERT into SINHVIEN (MASV,HOTEN,MALOP,TENDN,MATKHAU)
VALUES (N'SV01',N'Sinh Vien A','LOP01',N'sva',HASHBYTES('MD5','1'));
INSERT into SINHVIEN (MASV,HOTEN,MALOP,TENDN,MATKHAU)
VALUES (N'SV02',N'Sinh Vien B','LOP01',N'svb',HASHBYTES('MD5','1'));
INSERT into SINHVIEN (MASV,HOTEN,MALOP,TENDN,MATKHAU)
VALUES (N'SV03',N'Sinh Vien C','LOP02',N'svc',HASHBYTES('MD5','1'));
-- tạo lớp học
INSERT into LOP (MALOP,TENLOP,MANV)
VALUES (N'LOP01',N'Lop 1','NV01');
INSERT into LOP (MALOP,TENLOP,MANV)
VALUES (N'LOP02',N'Lop 2','NV02');
...
begin
	...
	IF EXISTS(select * from NHANVIEN WHERE TENDN=@tendangnhap and MATKHAU=HASHBYTES('SHA1',@matkhau))
	BEGIN
...
BEGIN
	...
	BEGIN
		select @res = MANV from NHANVIEN WHERE TENDN = @tendangnhap
	END
	ELSE
	BEGIN
		set @res = ''
...
if OBJECT_ID('dsLop') is not null
	drop proc dsLop
go
CREATE proc dsLop @token varchar(MAX)
as
begin
	...
	begin
		...
		begin
			select * from LOP where MANV = @MaNV
		end
	end
end